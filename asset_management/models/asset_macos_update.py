...
class AssetMacosUpdate(models.Model):
    _name = 'asset.macos.update'
    _description = 'Asset macOS Update'
    ...
    package_name  = fields.Char(string='Package / App Name', required=True, index=True)
    ...
    source        = fields.Char(string='Source', help='e.g. App Store, Homebrew, Direct')
    ...
    def action_block_update(self):
        ...
        _logger.info(f'[MU] {self.package_name} blocked by {self.env.user.name}')
        return True
    ...
    def action_push_update(self):
        ...
        _logger.info(f'[MU] {self.package_name} queued for install by {self.env.user.name}')
        return True
    ...
    def action_uninstall_update(self):
        ...
        _logger.info(f'[MU] {self.package_name} queued for uninstall by {self.env.user.name}')
        return True


class AssetAssetMacosUpdate(models.Model):
    _inherit = 'asset.asset'

    macos_update_locked = fields.Boolean(
        string='macOS Updates Locked', default=False, tracking=True,
    )
    macos_update_ids = fields.One2many(
        'asset.macos.update', 'asset_id', string='macOS Updates',
    )
    macos_update_count = fields.Integer(
        string='macOS Update Count', compute='_compute_macos_update_count',
    )
    macos_pending_updates = fields.Integer(
        string='macOS Pending Updates', compute='_compute_macos_update_count',
    )

    @api.depends('macos_update_ids', 'macos_update_ids.status')
    def _compute_macos_update_count(self):
        for asset in self:
            asset.macos_update_count   = len(asset.macos_update_ids)
            asset.macos_pending_updates = len(
                asset.macos_update_ids.filtered(lambda u: u.status == 'pending')
            )

    def action_toggle_macos_lock(self):
        self.ensure_one()
        if not self.env.user.has_group('asset_management.group_asset_manager'):
            raise UserError('Only Asset Managers can lock/unlock macOS updates.')
        self.macos_update_locked = not self.macos_update_locked
        return True

    def action_lock_all_macos_updates(self):
        return self.action_toggle_macos_lock()

    def get_macos_update_data(self):
        """Return all data needed by the macOS update OWL widget in one call."""
        self.ensure_one()

        is_admin = self.env.user.has_group('asset_management.group_asset_manager')

        updates = []
        for u in self.macos_update_ids:
            updates.append({
                'id':              u.id,
        ...
        activity_log = []
        for u in self.macos_update_ids.filtered(lambda u: u.action_date).sorted(
        ...
        return {
            ...
            'is_locked':     self.macos_update_locked,
            'is_admin':      is_admin,
            'update_count':  self.macos_update_count,
            'pending_count': self.macos_pending_updates,
            'updates':       updates,
