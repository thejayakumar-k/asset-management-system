...
class WindowsUpdateWidget extends Component {
    static template = "asset_management.WindowsUpdateWidget";
    static props = {
        ...standardFieldProps,
    ...
    async loadData() {
        ...
        try {
            ...
            const data = await this.orm.call("asset.asset", "get_windows_update_data", [[assetId]]);
            ...
            console.error("WU loadData error:", e);
            this.notification.add("Failed to load Windows update data", { type: "danger" });
        } finally {
    ...
    async toggleLock() {
        ...
        try {
            await this.orm.call("asset.asset", "action_lock_all_updates", [[this.state.assetId]]);
    ...
    async pushUpdate(ev, updateId) {
        ev.stopPropagation();
        try {
            await this.orm.call("asset.windows.update", "action_push_update", [[updateId]]);
    ...
    async blockUpdate(ev, updateId) {
        ev.stopPropagation();
        try {
            await this.orm.call("asset.windows.update", "action_block_update", [[updateId]]);
    ...
    async allowUpdate(ev, updateId) {
        ev.stopPropagation();
        try {
            await this.orm.call("asset.windows.update", "action_allow_update", [[updateId]]);
    ...
    async uninstallUpdate(ev, updateId) {
        ev.stopPropagation();
        try {
            await this.orm.call("asset.windows.update", "action_uninstall_update", [[updateId]]);
    ...
    getCardClass(status) {
        const map = {
            blocked:      'wu-card-blocked',
            failed:       'wu-card-failed',
            installed:    'wu-card-installed',
            uninstalled:  'wu-card-uninstalled',
            installing:   'wu-card-installing',
            uninstalling: 'wu-card-installing',
        };
        return map[status] || '';
    }
}

registry.category("fields").add("windows_update_widget", {
    component: WindowsUpdateWidget,
    supportedTypes: ["one2many"],
});

export { WindowsUpdateWidget };