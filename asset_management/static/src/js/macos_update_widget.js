...
class MacosUpdateWidget extends Component {
    static template = "asset_management.MacosUpdateWidget";
    static props = {
        ...standardFieldProps,
    ...
    async loadData() {
        ...
        try {
            ...
            const data = await this.orm.call("asset.asset", "get_macos_update_data", [[assetId]]);
            ...
            console.error("MU loadData error:", e);
            this.notification.add("Failed to load macOS update data", { type: "danger" });
        } finally {
    ...
    async toggleLock() {
        ...
        try {
            await this.orm.call("asset.asset", "action_lock_all_macos_updates", [[this.state.assetId]]);
    ...
    async pushUpdate(ev, updateId) {
        ev.stopPropagation();
        try {
            await this.orm.call("asset.macos.update", "action_push_update", [[updateId]]);
    ...
    async blockUpdate(ev, updateId) {
        ev.stopPropagation();
        try {
            await this.orm.call("asset.macos.update", "action_block_update", [[updateId]]);
    ...
    async allowUpdate(ev, updateId) {
        ev.stopPropagation();
        try {
            await this.orm.call("asset.macos.update", "action_allow_update", [[updateId]]);
    ...
    async uninstallUpdate(ev, updateId) {
        ev.stopPropagation();
        try {
            await this.orm.call("asset.macos.update", "action_uninstall_update", [[updateId]]);
...
registry.category("fields").add("macos_update_widget", {
    component: MacosUpdateWidget,
    supportedTypes: ["one2many"],
});

export { MacosUpdateWidget };
